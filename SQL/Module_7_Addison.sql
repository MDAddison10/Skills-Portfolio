...
-- Task 1a --
DROP VIEW IF EXISTS SMALL_PROPERTY;
...
SELECT OFFICE_NUM, PROPERTY_ID, MONTHLY_RENT
	FROM PROPERTY
		WHERE (MONTHLY_RENT >= 1150) AND (SQR_FT >= 1250);
...
CREATE VIEW PROPERTY_OWNERS AS
	(SELECT PROPERTY_ID, OFFICE_NUM, SQR_FT, BDRMS, FLOORS, MONTHLY_RENT, LAST_NAME
		FROM PROPERTY, OWNER
			WHERE (PROPERTY.OWNER_NUM = OWNER.OWNER_NUM) AND (BDRMS = 3)
	);
    
SELECT * 
	FROM PROPERTY_OWNERS;
-- Task 2b --
SELECT OFFICE_NUM, PROPERTY_ID, MONTHLY_RENT, SQR_FT, LAST_NAME
	FROM PROPERTY_OWNERS
		WHERE (MONTHLY_RENT < 1675);
-- Task 2c --        
SELECT OFFICE_NUM, PROPERTY_ID, SQR_FT, BDRMS, FLOORS, MONTHLY_RENT, LAST_NAME
	FROM PROPERTY, OWNER
		WHERE (PROPERTY.OWNER_NUM = OWNER.OWNER_NUM) AND (BDRMS = 3) AND (MONTHLY_RENT < 1675);
        
-- Task 3a --
DROP VIEW IF EXISTS MONTHLY_RENTS;
CREATE VIEW MONTHLY_RENTS AS
	(SELECT BDRMS, AVG(SQR_FT) AS AVG_SQUARE_FEET, AVG(MONTHLY_RENT) AS AVERAGE_RENT
		FROM PROPERTY
			GROUP BY BDRMS
			ORDER BY BDRMS
	);
SELECT *
	FROM MONTHLY_RENTS;
-- Task 3b --
SELECT AVG_SQUARE_FEET, AVERAGE_RENT
	FROM MONTHLY_RENTS
		WHERE (AVERAGE_RENT > 1100);
-- Task 3c --        
SELECT AVG_SQUARE_FEET, AVERAGE_RENT
	FROM (SELECT BDRMS, AVG(SQR_FT) AS AVG_SQUARE_FEET, AVG(MONTHLY_RENT) AS AVERAGE_RENT
		FROM PROPERTY
			GROUP BY BDRMS
            ORDER BY BDRMS
		) AS AVERAGE_RENT
...
-- Task 4c --
GRANT UPDATE (MONTHLY_RENT) TO JOHNSON, KLEIN;
-- Task 4d --
GRANT SELECT (OFFICE_NUM, MONTHLY_RENT, OWNER_NUM) ON PROPERTY TO PUBLIC;
-- Task 4e --
GRANT UPDATE ON SERVICE_CATEGORY TO KLEIN;
GRANT DELETE ON SERVICE_CATEGORY TO KLEIN;
-- Task 4f -- 
GRANT INDEX ON SERVICE_REQUEST TO ADAMS;
-- Task 4g --
GRANT ALTER ON PROPERTY TO ADAMS, KLEIN;
-- Task 4h --
GRANT ALL ON OFFICE TO KLEIN;
GRANT ALL ON OWNER TO KLEIN;
GRANT ALL ON PROPERTY TO KLEIN;

-- Task 5 --
REVOKE ALL PRIVILEGES ON ADAMS; 
...
SELECT *
	FROM INFORMATION_SCHEMA.TABLES
		WHERE (TABLE_SCHEMA = 'STAYWELL') AND (COLUMN_NAME = 'OWNER_NUM');
